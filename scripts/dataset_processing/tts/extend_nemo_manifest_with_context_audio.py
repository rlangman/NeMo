...
class SpeakerShardedAudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        audio, audio_length = self.get_wav_from_filepath(
            audio_filepath_rel=item_info["audio_filepath"], duration_in_sec=item_info["duration"]
        )
...
class EmbeddingSimilarityExtractorSharded(pl.LightningModule):
    def __init__(
        ...
        self.context_max_ssim = context_max_ssim
...
def main():
    ...
    parser.add_argument(
        "--context-max-ssim", type=float, default=0.98, help="Max cosine similarity for a context audio segment."
    )
    parser.add_argument(
        "--max-speaker-items",
        type=int,
    ...
    model = EmbeddingSimilarityExtractorSharded(
        ...
        context_max_ssim=args.context_max_ssim,
