...
def get_args():
    ...
    parser.add_argument(
        ...
        help="Whether to take the log of the features.",
    )
    parser.add_argument(
...
def _compute_stats(values: List[np.ndarray], use_log: bool) -> Tuple[float, float]:
    values_array = np.concatenate(values, axis=0)
    if use_log:
        values_array = np.log(1.0 + values_array)
...
def main():
    ...
    for feature_name in feature_names:
        ...
        for speaker_id, values in feature_stat_dict.items():
            speaker_mean, speaker_std = _compute_stats(values, use_log=use_log)
