...
class SLMEncoder(NeuralModule):
    ...
    def __init__(
        ...
        if input_sr == self.slm_sr:
            ...
            self.resample = torchaudio.transforms.Resample(input_sr, self.slm_sr)
        else:
...
class ResidualBlockV2(NeuralModule):
    ...
    def __init__(
        ...
        if not is_causal:
            ...
            self.input_conv = CausalConv1dNorm(
                in_channels=channels, out_channels=filters, kernel_size=kernel_size, activation=activation, pad_mode=pad_mode
            )
