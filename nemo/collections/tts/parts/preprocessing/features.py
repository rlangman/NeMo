...
def _get_numpy_feature_filepath(
    manifest_entry: Dict[str, Any], audio_dir: Path, feature_dir: Path, feature_name: str, manifest_field: str
) -> Path:
    """
    ...
    """
    _, audio_filepath_rel = get_audio_filepaths(
        manifest_entry=manifest_entry, audio_dir=audio_dir, field_name=manifest_field
    )
...
class NumpyFeatureReader(FeatureReader):
    ...
    def __init__(
        ...
        manifest_field: str = "audio_filepath",
    ):
...
class PitchFeaturizer(Featurizer):
    ...
    def compute_pitch(
        ...
        if not self.batch_samples or audio.shape[0] < self.batch_samples:
            ...
            pitch_len = math.ceil(audio.shape[0] / self.hop_length)
            pitch = pitch[:pitch_len]
            voiced_mask = voiced_mask[:pitch_len]
            voiced_prob = voiced_prob[:pitch_len]
        else:
