...
class DiscreteSpeechArtifactGenerator(ArtifactGenerator):
    ...
    def __init__(
        ...
        if self.acoustic_model_type == "default":
            ...
            from nemo.collections.tts.models import AcousticModelAutoregressiveWithText

            self.acoustic_model = AcousticModelAutoregressiveWithText.restore_from(acoustic_model_path)
        elif self.acoustic_model_type == "context":
            from nemo.collections.tts.models import AcousticModelAutoregressiveWithContext

            self.acoustic_model = AcousticModelAutoregressiveWithContext.restore_from(acoustic_model_path)
        else:
        ...
        with torch.no_grad():
            ...
            if self.acoustic_model_type == "default":
                audio_tokens_pred = acoustic_model.infer(
                    ...
                    semantic_lens=audio_token_lens,
                    frames_per_iter=self.frames_per_iter,
                    num_audio_iters=self.num_iters,
                    audio_topk=self.audio_topk,
                    audio_temperature=self.audio_temperature,
                )
            else:
...
class DiscreteSpeechAcousticArtifactGenerator(ArtifactGenerator):
    """
    Generator for logging DiscreteSpeech model outputs.
