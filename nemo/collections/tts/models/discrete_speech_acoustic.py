...
@experimental
class DiscreteSpeechAcousticModel(ModelPT):
    ...
    def training_step(self, batch_dict, batch_idx):
        ...
        semantic_token_loss = self.semantic_token_loss_fn(
            logits=semantic_logits, target_tokens=semantic_token_sample, mask=audio_mask
        )
        ...
        semantic_token_loss_pre = self.semantic_token_loss_fn(
            logits=semantic_logits_pre, target_tokens=semantic_token_sample, mask=audio_mask
        )
        ...
        acoustic_token_loss = self.acoustic_token_loss_fn(
            logits=acoustic_logits, target_tokens=acoustic_token_sample, mask=audio_mask
        )
    ...
    def validation_step(self, batch_dict, batch_idx):
        ...
        acoustic_token_correct = (acoustic_token_sample == acoustic_tokens_pred) * rearrange(
            audio_mask, 'B T -> B 1 T'
        )
        acoustic_token_accuracy = (
            acoustic_token_correct.sum() / audio_token_sample_lens.sum() / self.acoustic_codebook_num
        )
    ...
    def _duration_infer(
        ...
        if num_iters > 0:
            ...
            audio_tokens = self.duration_decoder.infer(
                inputs=inputs,
                text_lens=text_lens,
                frames_per_iter=frames_per_iter,
                infer_weight=infer_weight,
    ...
    def infer(
        ...
        audio_lens = torch.clamp_max(audio_lens, max=max_infer_length)
