...
class MagpieTTSModel(ModelPT):
    ...
    def add_special_tokens(self, codes, codes_len, bos_id, eos_id, num_bos_tokens=1):
        # codes: (B, C, T')
        # codes_len: (B,)
        codes = torch.nn.functional.pad(input=codes, pad=(num_bos_tokens, 0), value=bos_id)
        codes = torch.nn.functional.pad(input=codes, pad=(0, 1), value=0)
        codes_len = codes_len + num_bos_tokens + 1
    ...
    def embed_audio_tokens(self, audio_tokens, audio_tokens_lens):
        ...
        audio_tokens = self.pad_audio_codes(audio_tokens)
    ...
    def prepare_context_tensors(self, batch):
        ...
        if self.model_type == 'single_encoder_sv_tts':
            ...
            context_audio_embedded, context_input_lens = self.embed_audio_tokens(
            ...
            if self.use_text_conditioning_encoder:
                ...
                context_input_lens = (
                    ...
                    + (1 - batch['has_text_context'].float()) * context_input_lens
                )  # (B,)
            ...
            if self.model_type == 'multi_encoder_context_tts':
                ...
                if self.model_type == 'decoder_context_tts':
                    context_embeddings = context_audio_embedded
                elif self.model_type == 'decoder_ce':
                    context_embeddings = self.context_encoder(
                        context_audio_embedded, context_mask, cond=None, cond_mask=None
                    )['output']
    ...
    def process_batch(self, batch):
        ...
        audio_codes_target_unstacked = self.pad_audio_codes(audio_codes_target_unstacked)
        ...
        if self.use_alignment_encoder and not disable_alignment_loss:
            ...
            with torch.no_grad():
                aligner_attn_hard = self.get_binarized_prior_matrix(
                    aligner_attn_soft, audio_codes_lens_target, context_tensors['text_lens']
                )
